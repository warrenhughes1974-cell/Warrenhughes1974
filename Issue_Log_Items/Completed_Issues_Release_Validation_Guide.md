...
**Last rebuilt:** 2026-08-26 (#146 PC/former-vanish 0561s out of ISRR always-on smoke).  
...
Faster smoke-only (still blocks on #106/#98/#71/#2/#59/#95/#135/#136/#21F + CLNT-HW + CLNT-RJ width-12 + **#143** + **#141** + **#75/#45 Bank Acct** + **#139 ISWL fees withheld** + **#145B vanish 0561s out of ISRR** + **#156 quikspec SOR_POL** + **#146 PC/former-vanish 0561s** + **#142 SL rider 9SUBLF**):
...
5. When closing a new issue or committing a conversion change: **add or update a row in this file in the same commit**, **and** register a fail-closed always-on smoke in `SMOKE_JOBS` (`tools/validators/validate_release_closed_issues.py`). Do not Close without the smoke.
...
| **143** | BF RPU `MUNIT` vs PPBENTYP Column DD | 23 remapped units still present; gold `9010757606C` = 19.10196 / $19,101.96; 82 aligned BF and 199 BA untouched; #55/#108A intact; after authorized QuikIswl seed `MDB = MUNIT×1000` on those 23. **FAIL blocks release.** Smoke: `python tools/validators/validate_issue143_smoke.py` |
| **141** | `quikspec.RESRVCAT` from PCOVR.PRODUCT_TYPE via PPBEN seq-1 | 5,083 filled; 0 blank; 0 `ISWLFE` on the policy field; gold `9010143726C`/`9010148272C` = `03`; `9010713704C` = `05` (plan HLOB stays ISWLFE). **FAIL blocks release.** Smoke: `python QLA_Migration/_validate_issue141_resrvcat.py` |
| **75 / 45** | PAC Bank Acct `quikmstr.MBANKNO` | Bill form 2 filled **≥2000** QLA-safe `ABA/ACCOUNT`; leftover blank **≤80**; traces `9010161748C`=`091303855/0000002000581`, `9010713704C`=`104000016/47374579`. `aba_routing_lookup.csv` must be in Source. **FAIL blocks release.** A blank PAC bank book is a drop, not a cut difference. Smoke: `python tools/validators/validate_issue75_mbankno.py` |
| **139** | ISWL/UNKNOWN policy fees withheld | ISWL phase-1 five fee fields = **0**; non-ISWL MANNLFEE>0 **≥2000**; gold `9010713704C` fees 0 and `MMODEPREM=41.71`; traditional `9010367131C` keeps `MANNLFEE=10.44`. **FAIL blocks release.** A later `quikridr` rebatch that skips withhold is a drop. Smoke: `python tools/validators/validate_issue139_policy_fee_suppression.py` |
| **145B** | Vanish 0561s out of QuikIsrr | No QuikIsrr / PS- clms / phase-0 clmp / type-8 benh on VB golds `9010815236C` / `9011050114C` / `9011069610C`; leftover non-VB still present (`9010761639C` $271; `9010760840C` $716.40); gold `MUNIT` unchanged. **FAIL blocks release.** A later QuikIsrr rebatch that puts VB 0561s back is a drop. Smoke: `python tools/validators/validate_issue145b_vb_isrr_exclude.py` |
| **156** | `quikspec.SOR_POL` = LifePRO source policy number | Every QuikSpec row filled; gold `9011050114C`=`9011050114`; `9010143726C`=`9010143726`; `901122D991C`=`901122D991`; `901ML8487C`=`901ML8487`. Must not copy the Issue #2 `C` key onto 10-digit policies. **FAIL blocks release.** A later `quikspec` rebatch that drops `SOR_POL` is a drop. Smoke: `python QLA_Migration/_validate_issue156_sor_pol.py` |
| **146** | PC/former-vanish 0561s out of QuikIsrr | 20 allowlist policies have 0 QuikIsrr / PS- clms / phase-0 clmp / type-8 benh; golds `9011077629C` / `9010817956C` / `9010808831C` MUNIT 5 / 5 / 25; keep `9010761639C` $271 and `9010760840C` $716.40. **FAIL blocks release.** A later QuikIsrr rebatch that puts those 0561s back is a drop. Smoke: `python tools/validators/validate_issue146_pc_isrr.py` |
| **142** | Active SL riders as `9SUBLF` in quikridr | quikplan has one `9SUBLF` (PAR=0); quikridr `9SUBLF` count **≥22** and **every** row `MVPU=0`; all 22 anchors present; 8 red golds match source APU (`9010886099C` 100 units / 26.34; `9010469666C` 10 / 2.50; `9011201237C` 25 / 11.935); outlier `9010782078C` MPREM=0. **FAIL blocks release.** A later quikridr rebatch that drops the SL riders (or re-emits them with VPU>0, duplicating insured amounts) is a drop. Smoke: `python tools/validators/validate_issue142_sl_rider.py` |

| **158** | PR gross-premium grids on their PCOVRSGT **SEQ 1** owner | Every PAAGERAT PR segment's QuikGps grid present on its SEQ 1 owning plan(s) and **absent** from plans that only reference the segment at another SEQ; **0** cells written by two different LifePRO segments. Riders/PUAs (e.g. `261PUA`, `280PUA`) must not carry a base premium grid; plans sharing a segment at SEQ 1 must both receive it. `quikplan.VARGP` must agree with the emitted grids (A7 / #140 also cover this — refresh VARGP after any rate re-emit). **FAIL blocks release.** A later rate rebatch that reverts to single-parent segment resolution is a drop. Rollback: `QLA_PR_SEGMENT_SLOT_OWNERSHIP=0`. Smoke: `python tools/validators/validate_issue158_pr_segment_ownership.py` |
| **PSUB** | PSUBSSEG era-banded rate generations (CV/RV/NP) | Every scope-manifest generation present in `rates/` with EFFDATE = LifePRO substitution band date (e.g. `1L10PR@19950101`, `17085M@19710101–19740101`, `719CTR@19950101`); every EXTRACT cell value-identical to merged PDAGE page expansion; PLAN_COPY generations byte-match their base grid; key rows (QuikPlTv/QuikPlCv) exist per generation; standard 19000101 generations untouched. **FAIL blocks release.** A later rate rebatch without the scope manifest (or with `psubsseg_substitution.enabled=false`) silently drops all era generations — that is the drop this smoke catches. Smoke: `python tools/validators/validate_psubsseg_substitution.py` |
| **PLAN-KEEP** | Older policy cut keeps newest plan/rates | When `QLA_VALUATION_DATE` is older than `newest_plan_rate_package.json` `package_valuation_date` (currently **20260831**), `quikplan.csv`, `QuikIswl.csv`, and every `rates/Quik*.csv` must be **byte-identical** to that manifest. A 6/30 (or other older) rebatch that rebuilds plan setup or rates is a drop. Same-or-newer valuation skips the hash check. **FAIL blocks release.** Smoke: `python tools/validators/validate_newest_plan_rates_kept.py` |
| **159** | L10/L14 plan-aware `quikridr.MUWCLASS` | L10 smokers are **SM** not ST (`9011190516C`); L14 N/Q/T are **NT/PQ/ST** not 00 (`9011206462C` / `9011207210C` / `9011208194C`); `5L0110` S stays **ST**. 1L1095/1L10OD/1L10PR phase-1 ST count = 0; 1L14SC 00 count = 0. **FAIL blocks release.** A later `quikridr` rebatch that calls `map_rider_uwclass` without `plan=` drops the #118 keys and zeroes CSO reserve. Smoke: `python tools/validators/validate_issue159_muwclass_plan_aware.py` |
...
| 37 | Age/Duration rate placement - fleet-wide | Resolution: QuikCvs age/duration placement matches LifePRO CV grids fleet-wide. **Mechanism revised v58.94-v58.95 (Warren approved 2026-08-13):** CV first-duration now read from PDAGE native pages (`load_cv_native_first`) keyed per exact subslice (coverage, sex, age, band, uw) with a (coverage, sex, age) min fallback, instead of the hardcoded `cv_lifepro_first_duration` proof-matrix guess; slices absent from PDAGE fall back to the legacy guess (flagged `cv_first_fallback`). The L14 identity freeze is retired (see row L14). | rates/QuikCvs | **Source:** PDAGE native CV grids (year = (DURATION-1)*10+col+1); Rate_Table extract carries compacted duration labels only. **How:** loaded QuikCvs cell-identical to PDAGE at offset 0. | python tools/validators/validate_rate_source_identity.py (CV leg-1) + spot-check QuikCvs | 170858/17085M/170588 100% at offset 0 (142 slices each); CV grid samples in Issue_37/ |
...
| 75 | Bank Acct / MBANKNO QLA validation | CLOSED v58.35. Resolution: Bank-draft quikmstr.MBANKNO is rebuilt from June PPCOM routing joined by account digits, emitting only a checksum-valid 9-digit ABA and a digits-only account (source leading zeros kept). Stats: bank-draft policies 2132; populated QLA-safe 2081; still blank 51. Always-on release/full-batch smoke 08/19/2026 (v58.98) so a later quikmstr rebatch cannot ship blank PAC Bank Acct. | quikmstr.MBANKNO / ABA | **Source:** PPCOM-backed `aba_routing_lookup.csv` in Source (checksum-valid ABA / digits-only account) + PPACH/PPPAC for the active cut. **How:** Bank-draft MBANKNO rebuilt as 9-digit ABA/account. Fail if PAC filled < 2000 or traces blank. | python tools/validators/validate_issue75_mbankno.py | 9010161748C=091303855/0000002000581; 9010713704C=104000016/47374579 |
...
| 89 | Policy fee wipe after quikridr-only rebatch (MANNLFEE / modal fees) | Resolution: Policy fees now load from LifePRO on every quikridr emit (including ridr-only rebatches), with a fail-closed guard so a blank fleet fee wipe cannot ship again; annual and modal fees are restored on fee-bearing policies. **ISWL/UNKNOWN zeros are #139 (Warren 2026-08-11, re-locked 2026-08-19 after a drop); #89 still blocks a confirmed non-ISWL fee wipe.** | quikridr MANNLFEE / modal fees | **Source:** LifePRO policy fee fields. **How:** Fees reload on every quikridr emit (including ridr-only); fail-closed if blank wipe. Under #139, ISWL/UNKNOWN stay 0; non-ISWL must remain populated. | fee-bearing **non-ISWL** policies non-blank MANNLFEE | 9010367131C MANNLFEE=10.44 (non-ISWL); ISWL 9010713704C fees 0 (#139) |
...
| 139 | Policy fees withheld for ISWL/UNKNOWN only | 08/19/2026 Resolution: ISWL policy fees are withheld on the load (annual and modal fees set to zero, and billed premium no longer includes that fee). Traditional policies still keep their fees. Always-on smoke after 08/19 drop: root `app.py` must call withhold after fee load; convert-time abort + `SMOKE_JOBS`. | quikridr MANNLFEE, MSEMIFEE, MQTRLFEE, MMTHDFEE, MMTHBFEE; quikmstr MMODEPREM | **Source:** LifePRO `POLICY_FEE` / fee-inclusive `MODE_PREMIUM`; class from phase-1 MPLAN allowlist. **How:** ISWL/UNKNOWN five fee fields = 0 and `MMODEPREM` reduced by the mode fee; non-ISWL fees retained where #21C/#58 apply. Fail if ISWL fee>0 or non-ISWL MANNLFEE>0 < 2000. | python tools/validators/validate_issue139_policy_fee_suppression.py | 9010713704C fees 0 / MMODEPREM 41.71; 9010367131C MANNLFEE 10.44 |
| 145B | Vanish 0561s out of ISRR | 08/23/2026 Resolution: Vanish-policy 0561 history is no longer loaded as a surrender, so anniversary does not cut those units. Non-vanish leftovers stay. | QuikIsrr; quikclms PS- / phase 0; quikclmp phase 0; quikbenh type 8 | **Source:** PPOLC.BILLING_REASON=VB + PACTG 0561. **How:** VB 0561s excluded from #34 emit; leftover non-VB remain. Golds have 0 QuikIsrr; #146 examples keep their rows; MUNIT unchanged. | python tools/validators/validate_issue145b_vb_isrr_exclude.py | 9010815236C / 9011050114C / 9011069610C 0 ISRR; 9010761639C $271; 9010760840C $716.40 |
| 156 | Source Policy Number on User Defined | 08/26/2026 Resolution: The original LifePRO policy number now loads on the policy User Defined field, while QLAdmin still uses the number with C. | quikspec.SOR_POL | **Source:** PPOLC.POLICY_NUMBER. **How:** Normalize/trim only; do not append Issue #2 C. Every QuikSpec row filled. Template field C(10). | python QLA_Migration/_validate_issue156_sor_pol.py | 9011050114C=9011050114; 9010143726C=9010143726; 901122D991C=901122D991; 901ML8487C=901ML8487 |
| 146 | Non-VB Unit Reductions | 08/26/2026 Resolution: For 20 former-vanish policies, old premium-from-fund withdrawals are no longer loaded as surrenders, so anniversary will not cut the units. Real partial surrenders stay. | QuikIsrr; quikclms PS- / phase 0; quikclmp phase 0; quikbenh type 8 | **Source:** Locked allowlist (19 PPOLC BILLING_REASON=PC + 9010808831). **How:** Same #34 exclude as #145B, scoped to those 20 keys. Keep 9010761639 $271 and 9010760840 $716.40. Do not set VANISH. | python tools/validators/validate_issue146_pc_isrr.py | 9011077629C 0 ISRR units 5; 9010808831C 0 ISRR units 25; 9010761639C $271 |
...
| 98 | CV Endpoint Off By One (010398471C / 17085M) - #41 follow-up | Resolution: GL85 CV duration placement now starts .06 in year 3 for male issue ages 1-17 and keeps the age-100 terminal 1000 (Eric 010398471C / 17085M M age 14). **Generalized v58.94-v58.95 (Warren approved 2026-08-13):** placement for every sex/age/band/uw subslice now comes from the PDAGE native first non-zero year (fixed residual per-age shifts, e.g. GL85 F/0 first CV 6.04 now at year 6 not 7, terminal 1000 at year 100 restored; subslice keys fixed 1L10OD F/3 UW B vs S starting years). Val+Reg PASS; reload Test_Validation/rates/QuikCvs.csv. | rates/QuikCvs | **Source:** PDAGE native CV grid for 670 GL85-8 (and fleet). **How:** every QuikCvs slice cell-identical to PDAGE at offset 0; anchors M/14 .06@3, 975.61@85, 1000@86. | python Issue_Log_Items/Issue_98/validate_issue98_quikcvs_endpoint.py + python tools/validators/validate_rate_source_identity.py | 010398471C / 17085M M age 14; 170858 F age 0 (6.04@6, 1000@100) |
| 104 | Validated Advance Loan Pilot Cohort | **Pilot (not universal loan rule).** For allowlisted EXACT/ROUNDING policies only (`Issue_Log_Items/Issue_104/Issue_104_Validated_Advance_Loans.csv`), after runtime checks ORIG+ADDED≈LOAN_BALANCE and LOAN_BALANCE×(1−rate)≈ORIG (≤$0.02), emit MLOANPRIN=MLOANBAL=round(LOAN_BALANCE×(1−rate),2). Non-cohort and runtime failures keep gross LOAN_BALANCE. Flag `QLA_ISSUE104_VALIDATED_LOAN_BACKOUT` default 1; set 0 to rollback. v58.93. | quikloan.MLOANPRIN, quikloan.MLOANBAL | **Source:** PLOAN LOAN_BALANCE / ORIG_LOAN_AMOUNT / LOAN_AMT_ADDED / INTEREST_RATE. **How:** Cohort adjusted only when allowlisted + both runtime formula checks pass; non-cohort must equal gross LOAN_BALANCE; smoke is cut-agnostic (do not hardcode 176 forever). | python tools/validators/validate_issue104_loan_pilot.py | 9010331768C → 3331.46/3331.46 (gross 3506.80 @ 5%) |
...
| 143 | Units Incorrect (RPU) | 08/18/2026 Resolution: Issue #143 is Closed in v58.96. The 23 SME-authorized BF Reduced Paid-Up policies now derive MUNIT from BF_CURRENT_DB / VALUE_PER_UNIT so QLAdmin Amount Ins reproduces the LifePRO paid-up death benefit. Validation, Regression, and final Smoke testing passed. The existing Issue #124 QuikIswl emit was subsequently executed, and all 23 affected ISWL records now store MDB = corrected MUNIT × 1000. Gold 9010757606C now contains MUNIT 19.10196 and MDB 19101.96. No unauthorized or unexplained differences remain. | quikridr.MUNIT (phase 1); QuikIswl.MDB after authorized #124 reseed | **Source:** PPOLC PAID_UP_TYPE=RU; PPBEN seq-1 NUMBER_OF_UNITS + VALUE_PER_UNIT; PPBENTYP seq-1 TYPE_CODE=BF + BF_CURRENT_DB (Column DD). **How:** If RU and BF and DD>0 and abs(units − DD/VPU)>0.01 then Output MUNIT = DD/VPU; Amount Ins = MUNIT×MVPU equals DD. After authorized existing #124 emit, QuikIswl MDB = corrected MUNIT×1000. Do not remap aligned BF or BA. | python tools/validators/validate_issue143_rpu_munit.py ; python tools/validators/validate_issue143_smoke.py | 9010757606C MUNIT 19.10196 / MDB 19101.96; 9010766847C 25→5.16341; 9010826422C 50→9.65590 |
| 141 | Reserve Category | 08/19/2026 Resolution: LifePRO reserve category now loads on the policy User Defined field, and plan tags like ISWLFE stay on the plan. Examples: 9010143726C reserve category 03; 9010148272C reserve category 03; 9010713704C reserve category 05 (plan still ISWLFE). | quikspec.RESRVCAT | **Source:** PCOVR.PRODUCT_TYPE via PPBEN BENEFIT_SEQ=1 PLAN_CODE (BA traditional; BF ISWL). **How:** Do not copy quikplan.PRODUCT. Every quikspec row RESRVCAT = PCOVR PRODUCT_TYPE for seq-1 plan; 0 ISWLFE on RESRVCAT; plan HLOB/PRODUCT/MKTG on 1658C1/1659C2 family stay ISWLFE. | python QLA_Migration/_validate_issue141_resrvcat.py | 9010143726C=03; 9010148272C=03; 9010713704C=05 |
| 142 | SL Policies (Active SL rider as 9SUBLF) | 08/29/2026 Resolution: Active Substandard Life (SL) rating rows now load as their own rider phase under plan 9SUBLF with value-per-unit zero, so the extra rating premium is visible without duplicating the insured amount (v59.04, Warren-approved override narrowing Issue #27 blanket SL suppression to non-active rows). All 22 active SL policies emit; the 8 premium-bearing red-font policies carry source ANN_PREM_PER_UNIT; outlier 9010782078C premium stays 0; terminated SL rows (46) remain suppressed with the Issue #27 audit. | quikplan 9SUBLF row; quikridr 9SUBLF phases (MVPU=0) | **Source:** PPBEN BENEFIT_TYPE=SL with STATUS_CODE=A (06/30: 22 rows). **How:** Each active SL row emits MPLAN=9SUBLF, MUNIT=source units, MVPU=0, MPREM=ANN_PREM_PER_UNIT; every 9SUBLF row must have MVPU=0 and count ≥22; non-active SL never emit. | python tools/validators/validate_issue142_sl_rider.py | 9010886099C 100u/26.34; 9010469666C 10u/2.50; 9011201237C 25u/11.935; 9010782078C MPREM 0 |
| 157 | L10 LP95 PR rates incorrect (client ticket) | 08/29/2026 Resolution: L10 LP95 now carries its own premium table (5.23 series) and L10 OLD / PRE97 carries the 4.75 series — they were swapped. Same root cause and same fix as Issue #158 (PCOVRSGT SEQ ignored). The twelve additional forms Eric was asked to confirm are the rest of the #158 movers. | rates/QuikGps (1L1095, 1L10OD, and the #158 plan list) | **Source:** PAAGERAT TYPE=PR for `L10 LP95` / `L10 PRE97`; PCOVRSGT SEQ 1 owners `1L1095` / `1L10OD`. **How:** `1L1095` CNTL 00 GP0 = 5.23 (not 4.75); `1L10OD` CNTL 00 GP0 = 4.75 (not 5.23). Covered by the #158 fail-closed smoke — do not ship a second independent job. | python tools/validators/validate_issue158_pr_segment_ownership.py | 1L1095 F/BL 5.23; 1L10OD F/BL 4.75 |
| 159 | L10/L14 Zero Reserve UW | 09/02/2026 Resolution: Policy underwriting class is mapped with the plan again so L10 smokers use SM and L14 uses NT/PQ/ST, matching the rate files after the UW-class remap. | quikridr.MUWCLASS | **Source:** PPBEN UNDERWRITING_CLASS + plan (form-aware #118 map). **How:** `map_rider_uwclass(letter, plan=MPLAN)`; L10 S→SM; L14 N/Q/T/R→NT/PQ/ST/PR; non-L10 S stays ST. Fail if 1L1095/1L10OD phase-1 ST>0 or 1L14SC 00>0. | python tools/validators/validate_issue159_muwclass_plan_aware.py | 9011190516C SM; 9011206462C NT; 9011207210C PQ; 9011059291C ST |
| 158 | PR rates attributed to the wrong QLAdmin plan | 08/29/2026 Resolution: Gross premium rate tables now attach to the plan LifePRO actually prices them on. Previously a premium grid could land on a rider or paid-up addition that merely referenced the same rate segment, and could reach only one plan even when two plans share it. 20 segments were re-attributed, 18 plans gained the premium grid they were missing, and the plans that should never have had one lost it. Rollback flag `QLA_PR_SEGMENT_SLOT_OWNERSHIP=0` (v59.05). | rates/QuikGps + QuikPlGp keys; QuikPlCv/Db/Dv/Gd/Tv/Uw member rows on plans gaining real grids; quikplan.VARGP | **Source:** PCOVRSGT (SEGT_FLAG=Y) `SEQ` slot — 1 = premium — joined to PAAGERAT TYPE_CODE=PR; corroborated independently by PSUBSSEG substitution lists. **How:** resolve each PR segment to **every** parent COVERAGE_ID carrying it at SEQ 1 (one-to-many), never to a single last-write-wins parent; assert no PR grid on a non-SEQ-1 plan and no cell written by two different segments. Re-run the R7B VARGP refresh after any rate re-emit or A7/#140 will fail. Shared-rate-candidate PR rows are dropped for plans that own a SEQ 1 segment (retires the pre-fix manifest workaround). | python tools/validators/validate_issue158_pr_segment_ownership.py | `961 ME65` → `2961ME` (was `261PUA`); `1L1095` / `1L10OD` BL grids no longer transposed; `9JPO10` loses foreign `8286 GI`; `976658` / `986JPO` gain real grids and drop NOT-APPLICABLE members |
| PSUB | PSUBSSEG coverage-ID substitution — era-banded rates (CV/RV/NP) | 09/01/2026 Resolution: LifePRO coverage-ID substitution (PSUBS/PSUBSSEG) now drives additive era-banded rate generations: policies issued in a substitution band read the substituted segment's rates via QLAdmin EFFDATE (latest EFFDATE ≤ issue date). New Era's 0831 extracts delivered all 13 previously-missing segments (PDAGE NP/RV for 11, PAAGE/PAAGERAT for 667 ART 95, PR page for 991 PWL73). Loader `qla_core/psubsseg_substitution_loader.py` emits from a reviewed scope manifest (50 entries: 42 EXTRACT / 8 PLAN_COPY) with full PDAGE page expansion; standard 19000101 generations untouched (v59.06). | rates/QuikTvs, QuikNps, QuikCvs + QuikPlTv/QuikPlCv era key rows; first-time plan keys 5667AT/719CDT | **Source:** PSUBS/PSUBSSEG extracts (band dates), merged PDAGE (values), PCOVRSGT defaults. **How:** independently recompute every scoped cell from PDAGE page expansion and assert value-identical presence in Output at the band EFFDATE; PLAN_COPY generations must byte-match base; V07 EFFDATE exemption limited to manifest (PLAN, EFFDATE) pairs. | python tools/validators/validate_psubsseg_substitution.py | 1L10PR@19950101 (6,192 NP + 6,708 TV rows); 17085M@19710101–19740101; 719CTR@19950101; 633,044 cells verified |
| L14 | L14 CV duration placement (identity freeze retired) | 08/13/2026 Resolution: The L14 identity freeze (extract Dur label = QuikCvs Dur) was retired with Warren's written approval after Eric's 'Rates of Identified Issues - 8.13.26' screenshots proved LifePRO places L14 M/54 CV on native years 3-46 (538.30 at Dur 24, terminal 1000 at Dur 46). L14 now flows through the PDAGE native-first remap like every coverage (v58.95); F/69 is unchanged (native first year 2), F/45 and M/54 shift +1 so terminal 1000 lands at attained age 100. | rates/QuikCvs (1L14SC) | **Source:** PDAGE native CV grid for coverage L14. **How:** QuikCvs 1L14SC cell-identical to PDAGE native years at offset 0. | python Issue_Log_Items/Issue_L14/validate_issue_l14_quikcvs_duration.py | 9011207563 M/54 Dur3=20.94 Dur24=538.30 Dur46=1000; F/69 Dur2=14.73 Dur31=1000; F/45 Dur3=8.37 Dur55=1000 |
| DV-NATIVE | QuikDvs dividends one duration early fleet-wide | 08/13/2026 Resolution: DV duration convention changed from source-1 to native identity (v58.95, Warren approved; same pattern as Issue 106 RV): LifePRO pays the year-N dividend from native Dur N, and every extract DV subslice's labels were proven equal to PDAGE native years (1,020/1,020 at shift 0), so QuikDvs Dur N now equals LifePRO Dur N; blank DV0 filled with numeric zero (mirrors #106 TV0 fill). Policy dollars: 185/186 LifePRO-paid dividends triangulate onto the loaded grid at offset 0 (was +1 shifted). | rates/QuikDvs (20 plans) | **Source:** PDAGE native DV grids. **How:** QuikDvs cell-identical to PDAGE at offset 0; ValxLife DIVIDEND_AMOUNT/units = grid value at native policy year. | python tools/validators/validate_rate_source_identity.py (DV leg-1 offset 0; leg-2 DV at 0) | 17085M M/3 18.52@56 19.03@57; 1960PO M/26 22.24@57 22.61@58 (333.60 = 22.24x15u paid 1/28/2026; next 339.15 = 22.61x15u) |
| PUA-CV | PUA plans missing CV factors | 08/13/2026 Resolution: New PAAGERAT PUA CV loader (qla_core/paagerat_cv_loader.py, config pua_cv, v58.95) emits attained-age CV per unit for 121PUA/165PUA/170PUA/185PUA/1OLPUA/1POPUA/1970PA on the Issue 140 slot axis (AGE=00, slot = attained age); 261PUA/265PUA/280PUA keep their inherited Rate_Table CV (excluded to avoid collisions). All 7 plans grid-identical to PAAGERAT at offset 0 (attained-100 cell caps to 99 per MAX_AGE, 1 cell/sex). | rates/QuikCvs + QuikPlCv keys (7 PUA plans) | **Source:** PAAGERAT TYPE=CV (SEQ = attained age) for '621 PUA','960 65 PUA','670 PUA','960 LP PUA','960 OL PUA','960 PO PUA','970 PUA'. **How:** loaded slot grid cell-identical to PAAGERAT attained-age vector. | python tools/validators/validate_rate_source_identity.py (CV leg-1 AA slices) | 170PUA M attained 60 = 667.90 (Eric implied 668.16 interpolated); 1POPUA M 84/85 = 837.29/844.99 (Eric implied 840.52) |
...
High-risk smoke (#106/#98/#71/#2/#59/#95/#135/#136/#21F + CLNT-HW + **#143**): PASS / FAIL
