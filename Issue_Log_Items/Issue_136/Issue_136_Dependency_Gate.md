...
|---|------------|--------|-------|
| 1 | Locked acceptance criteria (Warren + Luna) | **Met** | `Issue_#136_Locked_Acceptance_Criteria.md` |
...
| 4 | Conflict with Issue #77 Band/STVARYGP presence rule | **Resolved by supersession** | #136 supersedes #77 for Band and STVARYGP-on-presence only |
...
| Prior | New |
|-------|-----|
| Issue #77: `BDVARY*=Y` if family has any real key/rate | #136: Band only if real multi-band / non-default band differentiation |
| Issue #77: `STVARYGP=Y` if GP present | #136: State only if real multi-state/country differentiation |
| Issue #96: force `BDVARYCV/TV` when factor rows exist | #136: do not force Band from presence alone |
