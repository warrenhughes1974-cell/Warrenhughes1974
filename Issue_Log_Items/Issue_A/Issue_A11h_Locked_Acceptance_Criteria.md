# Issue A11h — Locked Acceptance Criteria (Real-rate-only variance)
...
Issue #77’s historical emit rule that set `BDVARY*=Y` whenever a family had any real row, and `STVARYGP=Y` whenever GP was present, is **superseded** by this A11h rule for Band and State.
