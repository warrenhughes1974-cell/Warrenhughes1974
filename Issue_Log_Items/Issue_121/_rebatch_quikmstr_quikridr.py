"""Headless quikmstr + quikridr rebatch for Issue #121 (ART no ETI)."""
import os
import sys
...
print("=== ISSUE #121 QUIKMSTR+QUIKRIDR REBATCH START ===", flush=True)
...
try:
    ...
    log_path = os.path.join(MIG, "Logs", "_issue121_quikmstr_quikridr_rebatch_log.txt")
    os.makedirs(os.path.dirname(log_path), exist_ok=True)
    with open(log_path, "w", encoding="utf-8") as fh:
        fh.write(log_text)
    print(f"Console log saved: {log_path}", flush=True)
    print("=== ISSUE #121 REBATCH COMPLETE ===", flush=True)
