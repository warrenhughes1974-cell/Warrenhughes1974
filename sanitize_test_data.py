#!/usr/bin/env python3
"""
Structure-preserving deterministic PII sanitizer for legacy insurance export CSVs.
...
Design:
  ...
  - Preserve file dialect, delimiters, line endings, field width
  - Deterministic: same source value -> same sanitized value across files/runs

Configuration: sanitization_config/field_masking_rules.json (v2)
"""
...
DEFAULT_RULES_PATH = os.path.join(SCRIPT_DIR, "sanitization_config", "field_masking_rules.json")
...
def sanitize_file_structure(input_path, output_path, rules, registry):
...
def run_batch(input_dir, output_dir, rules_path, patterns):
...
def main():
    parser = argparse.ArgumentParser(
        description="Structure-preserving PII sanitizer for legacy insurance export CSVs.",
    )
    parser.add_argument("--input", help="Single input CSV file")
    parser.add_argument("--output", help="Single output CSV file")
    ...
    try:
        ...
        if args.input and args.output:
            registry = PiiRegistry(rules.get("salt", "qla-enterprise-test-salt-v2"))
            result = sanitize_file_structure(args.input, args.output, rules, registry)
